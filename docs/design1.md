# Riksho — Introductory Website Blueprint

A complete, build-ready plan for the Riksho marketing/introductory site, structured the way inDrive's site is structured (hero → service grid → trust/safety → coverage → CTA → footer), but built entirely around Riksho's own brand system, services, and visual identity.
...
### 3.1 Navbar (sticky)
- Left: Riksho wordmark (indigo on white background)
...
**Subhead:** `Whether it's you, your parcel, or your entire delivery fleet — Riksho gets it there.`
...
**Headline:** `Why people and businesses choose Riksho`
...
- Body: `From single parcels to full truckloads — Riksho gives e-commerce sellers, SMEs, and enterprises on-demand access to verified vehicles and drivers, with live tracking on every dispatch.`
...
Columns: `Why Riksho` / `Earn with Riksho` (Ride, Bike Taxi, Courier, Fleet) / `Safety` / `Company` (About, Careers, Newsroom, Contact) / `Support`
